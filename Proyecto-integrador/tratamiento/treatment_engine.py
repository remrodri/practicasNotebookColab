...
def cargar_catalogo(ruta):
    ...
    if not data.get("version_catalogo") or not isinstance(data.get("reglas"), list):
        raise ValueError("Catálogo JSON sin versión o lista de reglas")
    reglas = pd.DataFrame(data["reglas"])
    requeridas = {"regla_id", "estado_inicial", "bloquea_silver", "justificacion"}
    if not requeridas.issubset(reglas.columns):
        raise ValueError("Catálogo de tratamiento incompleto")
    if not reglas["regla_id"].is_unique or not reglas["estado_inicial"].isin(ESTADOS).all():
        raise ValueError("Catálogo de tratamiento con reglas duplicadas o estado inválido")
    return reglas
...
def exportar(directorio, tablas, fuentes):
    """Exporta de forma idempotente y comprueba que las fuentes no cambiaron."""
    ...
    try:
        ...
        for destino, temporal in temporales.items():
            if destino.exists() and sha256(destino) == sha256(temporal):
                continue
